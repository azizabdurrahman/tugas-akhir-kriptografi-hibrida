...
class ModernSuite:
    def __init__(self):
        """Inisialisasi suite modern menggunakan X25519 dan ChaCha20-Poly1305.
        Setiap instance merepresentasikan SATU pihak (misalnya Alice atau Bob).
        """
        self.name = "Modern (X25519/ChaCha20)"
    ...
    def get_shared_key(self, peer_public_key):
        """Proses Key Exchange X25519 dan Derivasi Kunci menggunakan HKDF.
        Input: peer_public_key = kunci publik milik LAWAN BICARA (bukan milik sendiri).
        X25519 secara desain kebal terhadap timing attack karena menggunakan
        Montgomery Ladder dengan eksekusi waktu konstan (constant-time).
        Proses clamping pada kunci privat dilakukan secara otomatis oleh library.
        """
    ...
    def encrypt(self, key, plaintext):
        """Enkripsi data menggunakan algoritma AEAD ChaCha20-Poly1305."""
    ...
    def encrypt_with_nonce(self, key, nonce, plaintext):
        ...
        Penggunaan nonce tetap memastikan perbedaan ciphertext murni berasal
        dari perubahan kunci, bukan dari randomness nonce.
        """
        chacha = ChaCha20Poly1305(key)
        return chacha.encrypt(nonce, plaintext, None)

    def decrypt(self, key, nonce, ciphertext):
        """Dekripsi data menggunakan algoritma AEAD ChaCha20-Poly1305."""
        chacha = ChaCha20Poly1305(key)
...
if __name__ == "__main__":
    print("======================================================")
    print("   MODERN ENCRYPTION (X25519/ChaCha20-Poly1305)")
    print("   Simulasi Pertukaran Kunci Alice <-> Bob")
    print("======================================================")

    # 1. Menyiapkan folder penyimpanan
    os.makedirs("output_modern", exist_ok=True)
    ...
    alice = ModernSuite()
    bob = ModernSuite()
    print("[*] Alice berhasil membangkitkan pasangan kunci X25519.")
    print("[*] Bob   berhasil membangkitkan pasangan kunci X25519.")
    ...
    with open("key_pairs/modern_alice_private.pem", "wb") as f:
        f.write(alice_priv_pem)
    with open("key_pairs/modern_alice_public.pem", "wb") as f:
        f.write(alice_pub_pem)
    with open("key_pairs/modern_bob_private.pem", "wb") as f:
        f.write(bob_priv_pem)
    with open("key_pairs/modern_bob_public.pem", "wb") as f:
        f.write(bob_pub_pem)
    print("[*] Kunci Asimetris tersimpan di: key_pairs/modern_alice_*.pem & modern_bob_*.pem")

    # 4. Pertukaran Kunci Diffie-Hellman (X25519)
    #    Masing-masing pihak menghitung shared secret menggunakan:
    #    - Kunci PRIVAT miliknya sendiri
    #    - Kunci PUBLIK lawan bicara
    #    X25519 hanya menggunakan koordinat-x (Montgomery Ladder)
    print("\n[FASE 2] Pertukaran Kunci X25519 dan Derivasi HKDF")
    print("-" * 54)
    key_alice = alice.get_shared_key(bob.public_key)    # Alice pakai pubkey Bob
    key_bob = bob.get_shared_key(alice.public_key)      # Bob pakai pubkey Alice
    ...
    key_path = "session_keys/modern_current.key"
    ...
    #    Alice bertindak sebagai pengirim (mengenkripsi pesan untuk Bob)
